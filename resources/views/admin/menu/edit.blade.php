...
<body>
  <div class="card">
    ...
    <form method="POST" action="{{ route('admin.menu.update', $menuItem->id) }}">
      ...
      <div class="form-row">
        <label>Name</label>
        <input name="name" class="form-input" value="{{ old('name', $menuItem->name) }}" required>
      </div>

      <div class="form-row">
        <label>Description</label>
        <textarea name="description" class="form-input" rows="3">{{ old('description', $menuItem->description) }}</textarea>
      </div>

      <div style="display:flex;gap:12px">
        <div style="flex:1">
          <label>Price</label>
          <input name="price" type="number" step="0.01" class="form-input" value="{{ old('price', $menuItem->price) }}" required>
        </div>
        <div style="width:200px">
          <label>Category</label>
          <input name="category" class="form-input" value="{{ old('category', $menuItem->category) }}">
        </div>
      </div>
